...
struct Array diff_unsorted(struct Array arr1, struct Array arr2){
    
    //NOTE: we are doing arr1-arr2 here.

    struct Array result = {{0},10,0};
    int k=0;//through the new array.
    
    for(int i = 0 ; i<arr1.length ; i++){
        for(int j = 0 ; j<arr2.length ; j++){
            if(arr1.A[i]==arr2.A[j]){
                j=0;//we set the iterator from the start again.
                i++;//and get ready to search for the next element.
            }else if(j==arr2.length-1 && arr1.A[i]!=arr2.A[arr2.length-1]){
                //meaning this bracket will open when the it reaches the last element of arr2
                //and still that last element is not present in arr2.
                //then we add this to the new element.
                result.A[k++]=arr1.A[i];
                result.length++;
            }
        }
...
struct Array diff_sorted(struct Array arr1,struct Array arr2){
    ...
    while(i < m && j < n){
        if(arr1.A[i]<arr2.A[j]){
            ...
            i++;//skip both iteraters.
            j++;//since we want to remove all the common elements.
        }
    }

    //BUT OUR JOB IS NOT DONE HERE!!!
    //Let's say we finish the second array early and first array has elements left in it.
    //We will also return the rest of elements, since those elements are not present in 
    //second array.
